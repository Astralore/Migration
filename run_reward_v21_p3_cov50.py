"""
Reward v2.1 P3 cov50：B11b + Direction A + Reactive 结构改进 (E/B/A').

E: CF gate 用 1.0x 迁移成本评分（MARL_CF_PHYSICAL_MIGRATION_COST=1），报表 total_cost 仍 1.5x
B: Reactive trigger context 第三维 = 1 + min(excess/SLA, 2)
A': Direction A 下 per-agent urgency-weighted distance bonus（MARL_REACTIVE_DENSE_BONUS=1）
C': 可选 MARL_CF_TRAIN_SCORE_FLOOR（训练期，默认不设置）

默认 8 MARL epoch。2 epoch 快验：
  set MEDIUM_VALIDATION_MARL_EPOCHS=2
  python -u run_reward_v21_p3_cov50.py

Legacy Direction A only（无 E/B/A'）：
  set MARL_CF_PHYSICAL_MIGRATION_COST=0
  set MARL_REACTIVE_DENSE_BONUS=0
"""
...
# E: CF physical migration cost for gate (default on)
if "MARL_CF_PHYSICAL_MIGRATION_COST" not in os.environ:
    os.environ["MARL_CF_PHYSICAL_MIGRATION_COST"] = "1"

# A': Reactive dense bonus with Direction A (default on when train_total_cost)
